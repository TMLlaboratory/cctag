...
| Slack→エージェントへの画像・ファイル添付 | ✅ 本物の画像添付になる | ⚠️ *(パス渡しのみ — 自動添付は未検証)* |
| エージェント→Slackへのファイル送信（`.cctag/outbox`） | ✅ | ✅ |
| transcriptからの送信ファイル自動検出 | ✅ `Write`の書き込み先 | — *(書き込みがshellコマンド経由のため)* |
...
- **transcriptからの自動検出** — エージェントが`Write`ツールで書いた画像・SVG・PDFは、outboxを
  経由せずに拾われる。実際に完了した書き込みだけが対象で、権限プロンプトを拒否した場合は
  ディスク上のファイルは変更されていないため、cctagもそれに触らない。ソースファイルと`.md`は
  意図的に除外している: エージェントは常にそれらを書いており、いちいちアップロードすると
  スレッドが埋まってしまう。
