...
- **transcriptからの検出** — `Write`ツールの`file_path`のうち画像・SVG・PDFのみ。
  `Write`だけを見るのは、出力先が入力に明示される唯一のツールだからです。Bash経由で
  生成されるファイル（matplotlibのPNGなど）はtranscriptから復元できず、そこがoutbox規約で
  埋めるギャップです。ソースや`.md`を除外しているのは、エージェントが常時書いており
  毎回アップロードするとスレッドが埋まるためです。

  重要なのは、**`tool_use`だけを見てはいけない**という点です。`Write`の要求と結果は別レコードで、
  しかも別のポーリングバッチに分かれて届きます。そして拒否された`Write`は
  **既存ファイルを一切変更しません** — 要求だけを信じると、人間が上書きを拒否したファイルを
  cctag自身が読み出して送信してしまいます（実測: 拒否は`tool_result`の`is_error: true`として
  記録され、ファイルはバイト単位で無変更）。そのため`WrittenFileTracker`が
  `tool_use_id`で要求と結果を突き合わせ、**成功した書き込みだけ**を候補にします。
